...
@endphp
<x-layouts.admin title="{{ $isEdit ? 'Edit Tender Entry' : 'Tender Entry' }} | Forest Inventory" heading="{{ $isEdit ? 'Edit Tender Entry' : 'Tender Entry' }}" subheading="Range finance voucher entry">
    <form id="tenderEntryForm" class="space-y-5" method="POST" action="{{ $isEdit ? route('finance.tender-entries.update', $entry) : route('finance.tender-entries.store') }}"
        data-tender-entry-form
        data-copy-url="{{ url('finance/tender-entry/copy') }}"
        data-redirect-url="{{ route('finance.tender-entries.index') }}"
        ...
        @if($isEdit)
            @method('PUT')
        @endif
        <div id="tenderEntryFormAlert" class="hidden rounded-lg border px-4 py-3 text-sm font-semibold"></div>

        <section class="rounded-lg border border-emerald-100 bg-white shadow-sm">
            <div class="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-4">
                <div>
                    <label class="form-label" for="entry_sr_no">Sr. No. (of this Tender Entry)</label>
        ...
        <section class="rounded-lg border border-emerald-100 bg-white shadow-sm">
            ...
            <div class="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-4">
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-slate-500" data-remaining-allotment>Remaining Allotment = -</p>
                </div>
                ...
                @foreach($items as $i => $item)
                    <div class="border-b border-emerald-100 p-5 last:border-b-0" data-item-row data-item-index="{{ $i }}">
                        ...
                        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                            <div>
                                <label class="form-label">SOR Code</label>
                                <input class="form-input" name="data[items][{{ $i }}][sor_code]" value="{{ $itemField($i, 'sor_code') }}" data-item-field>
                            </div>
                            ...
                            <div>
                                ...
                                <input class="form-input" name="data[items][{{ $i }}][work_order_no]" value="{{ $itemField($i, 'work_order_no') }}" data-item-field>
                            </div>
                            <div>
                                <label class="form-label">Work Order Date</label>
                                <input class="form-input" name="data[items][{{ $i }}][work_order_date]" value="{{ $itemField($i, 'work_order_date') }}" type="date" data-item-field>
                            </div>
                            <div>
                                <label class="form-label">Work Start Date</label>
                                <input class="form-input" name="data[items][{{ $i }}][work_start_date]" value="{{ $itemField($i, 'work_start_date') }}" type="date" data-item-field>
                            </div>
                            <div>
                                <label class="form-label">Work End Date</label>
                                <input class="form-input" name="data[items][{{ $i }}][work_end_date]" value="{{ $itemField($i, 'work_end_date') }}" type="date" data-item-field>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <input class="form-input" name="data[items][{{ $i }}][unit]" value="{{ $itemField($i, 'unit') }}" data-item-field>
                            </div>
                            <div>
                                <label class="form-label">SOR Rate</label>
                                <input class="form-input" name="data[items][{{ $i }}][sor_rate]" value="{{ $itemField($i, 'sor_rate') }}" type="number" min="0" step="any" data-item-field data-item-rate>
                            </div>
                            <div>
        ...
        <section class="rounded-lg border border-emerald-100 bg-white shadow-sm">
            ...
            <div class="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-4">
                ...
                <div>
                    ...
                    <input id="approved_percent" class="form-input" name="data[approved_percent]" value="{{ $field('approved_percent', 100) }}" type="number" min="0" max="100" step="any" data-entry-field data-approved-percent>
                </div>
                <div>
                    <label class="form-label">Amount as per Approved Rate</label>
                    <input class="form-input bg-stone-50" readonly data-approved-amount value="0">
                </div>
                <div>
                    <label class="form-label" for="additional_sgst">Additional SGST</label>
                    <input id="additional_sgst" class="form-input" name="data[additional_sgst]" value="{{ $field('additional_sgst', 0) }}" type="number" min="0" step="any" data-entry-field data-total-input>
                </div>
                <div>
                    <label class="form-label" for="additional_cgst">Additional CGST</label>
                    <input id="additional_cgst" class="form-input" name="data[additional_cgst]" value="{{ $field('additional_cgst', 0) }}" type="number" min="0" step="any" data-entry-field data-total-input>
                </div>
                <div>
                    <label class="form-label" for="additional_igst">Additional IGST</label>
                    <input id="additional_igst" class="form-input" name="data[additional_igst]" value="{{ $field('additional_igst', 0) }}" type="number" min="0" step="any" data-entry-field data-total-input>
                </div>
                <div>
                    <label class="form-label" for="additional_labour_cess">Additional Labour Cess</label>
                    <input id="additional_labour_cess" class="form-input" name="data[additional_labour_cess]" value="{{ $field('additional_labour_cess', 0) }}" type="number" min="0" step="any" data-entry-field data-total-input>
                </div>
                <div>
        ...
        <section class="rounded-lg border border-emerald-100 bg-white shadow-sm">
            ...
            <div class="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-3">
                ...
                <div>
                    <label class="form-label" for="deposit_deduction_amount">Deposit Deduction Amount from Bill</label>
                    <input id="deposit_deduction_amount" class="form-input bg-amber-50" name="data[deposit_deduction_amount]" value="{{ $field('deposit_deduction_amount', 0) }}" type="number" min="0" step="any" data-entry-field data-total-input>
                </div>
                <div>
        ...
        <section class="rounded-lg border border-emerald-100 bg-white shadow-sm">
            <div class="flex flex-wrap justify-end gap-3 p-5">
                ...
                <a class="secondary-button" href="{{ route('finance.tender-entries.index') }}">View Details</a>
