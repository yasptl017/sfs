...
@endphp
<x-layouts.admin title="{{ $isEdit ? 'Edit D. Wagers Salary' : 'D. Wagers Salary' }} | Forest Inventory" heading="{{ $isEdit ? 'Edit D. Wagers Salary' : 'D. Wagers Salary' }}" subheading="Range finance daily wager salary entry">
    <form id="dWagerSalaryForm" class="space-y-5" method="POST" action="{{ $isEdit ? route('finance.d-wager-salary-entries.update', $entry) : route('finance.d-wager-salary-entries.store') }}"
        data-d-wager-salary-form
        data-copy-url="{{ url('finance/d-wager-salary-entry/copy') }}"
        data-redirect-url="{{ route('finance.d-wager-salary-entries.index') }}"
        ...
        @if($isEdit)
            @method('PUT')
        @endif
        <div id="dWagerSalaryFormAlert" class="hidden rounded-lg border px-4 py-3 text-sm font-semibold"></div>

        <section class="rounded-lg border border-emerald-100 bg-white shadow-sm">
            <div class="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-4">
                <div>
                    <label class="form-label" for="entry_sr_no">Sr. No. (of this D.Wagers Entry)</label>
        ...
        <section class="rounded-lg border border-emerald-100 bg-white shadow-sm">
            ...
            <div class="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-4">
                ...
                <div>
                    ...
                    <input id="salary_month" class="form-input" name="data[salary_month]" value="{{ $field('salary_month') }}" data-entry-field>
                </div>
                <div class="md:col-span-2 xl:col-span-4">
                    ...
                    <p class="mt-1 text-xs text-slate-500">Will show when D.Wager and salary month are selected. Maximum characters: 150</p>
                </div>
            </div>
        </section>
        ...
        <section class="rounded-lg border border-emerald-100 bg-white shadow-sm">
            <div class="flex flex-wrap justify-end gap-3 p-5">
                ...
                <a class="secondary-button" href="{{ route('finance.d-wager-salary-entries.index') }}">View Details</a>
